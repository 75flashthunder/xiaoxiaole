...
$(document).ready(function() {
	...
	remove = function(list) {
		...
		$("#conmment").html('<span class="fontstyle" style="font-weight: bold;">' + '你好棒哦!!!'+ '</span>');
		$("#point").html('<span class="pointposition">' + '分数：'+ point + '</span>');

		while (point >= startpoint) {
			$("#star").html('<span class="pointposition">'+star+ '</span>');
			startpoint += 1000;
			star += '★';
			if (star == '★★★★') {
				star = '☾';
...
origan=function(){
	...
	for (var i = 1; i < 101; i++) {
		...
		'<img id='+i+' src="" width="38" height="38" />'+'</div>';		
	}
	$("#conten").html(block);
};
...
gotonextlevel=function(){
		$("#conmment").html('<span class="fontstyle1" style="font-weight: bold;">' + '超过2000分才可以进入下一关！'+ '</span>');
}